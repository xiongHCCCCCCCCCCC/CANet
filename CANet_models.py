...
class ACNet(nn.Module):
    def __init__(self, num_class=37, backbone='ResNet-101', pretrained=False, pcca5=False):
        ...
        block = Bottleneck  # Encoder
        transblock = TransBasicBlock  # Decoder
        ...
        self.layer3 = self._make_layer(block, 256, layers[2], stride=2) # use PSPNet extractors #layers[2]: 6
        ...
        self.layer1_d = self._make_layer(block, 64, layers[0]) #layers[0]: 3
        self.layer2_d = self._make_layer(block, 128, layers[1], stride=2) #layers[0]: 4
        self.layer3_d = self._make_layer(block, 256, layers[2], stride=2) #layers[2]: 6
        self.layer4_d = self._make_layer(block, 512, layers[3], stride=2) #layers[3]: 3
